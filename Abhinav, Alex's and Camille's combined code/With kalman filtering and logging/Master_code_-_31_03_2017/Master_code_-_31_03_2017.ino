...
float seaLevelPressure = 1028.75; // The value must be in hPa
...
double Kp_roll=2, Ki_roll=5, Kd_roll=0; // Biggest challenge is to determine these values
...
//Specify the links and initial tuning parameters for Roll PID
double Kp_pitch=2, Ki_pitch=5, Kd_pitch=0;  // Biggest challenge is to determine these values
...
double Time; // Remember unsigned long does not store negative numbers otherwise it is just a number
...
void setup() {
  ...
  calibrate(); // To calibrate the sensors, reduce the gyro zero error and make the height above ground zero instead of height above sea level
  Serial.println(" ");
  Serial.println(zero_error_gyro_x);
  Serial.println(zero_error_gyro_y);
  Serial.println(zero_error_gyro_z);
  Serial.println(zero_error_alt);
  Serial.println(" ");
  ...
  Time = micros();  // We update time in the beginning of looping i.e. after setup has completed
  Time_for_log = micros();
}
...
//  gyro_x = gyro.data.x; // No corrections applied
//  gyro_y = gyro.data.y;
//  gyro_z = gyro.data.z;
...
//  Serial.println(dt);
  ...
  // For instantaneous printing of angles, better observation of real time angle change than every second
...
//  Serial.print("\r\n");
  ...
  float bmp_altitude = bmp.pressureToAltitude(seaLevelPressure, bmp_pressure) - zero_error_alt; // The altitude now is with respect to the ground (more relevant)

  // Push data to slave arduino for storage every second using Runkun's approach with slight modification
  if(micros() - Time_for_log >= 1000000){
    ...
    String accel_data_string = String(a + "," + b + "," + c + ","); // String carrying acceleration data
    ...
    String gyro_angle_data_string = String(d + "," + e + "," + f + "," + g + "," + h + ","); // String carrying gyro, roll and pitch data
    ...
    String bmp_time_data_string = String(i + "," + j + "," + k + "," + l); // String carrying pressure, altitude, temperature and time data
    ...
    // Three successive transmissions to avoid incomplete transmission of data
    ...
    // Printing all data, should comment out when uploading to arduino before final flight as is unnecessary when in flight
...
//    Serial.println("Autopilot mode");
    ...
    // Just a small test using a servo to check proper relay switching
    ailerons.write(100);
    delay(100);
    ailerons.write(50);
    delay(100);
...
void calibrate(){
  // Calibrates gyro and pressure sensors by averaging values, reducing zero error of gyro does not seem to improve data......
...
void level_flight_roll_PID(double value, int setpoint){ // Roll PID control 
...
void level_flight_pitch_PID(double value, int setpoint){ // Pitch PID control
